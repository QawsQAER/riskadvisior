...
public class HealthCrawler {
	...
	public HealthCrawler() throws IOException, JSONException {
		JSONObject json = null;
		ArrayList<DataPackage> packageList = new ArrayList<DataPackage>();
		JSONArray results = null;
		json = readJsonFromUrl("http://catalog.data.gov/api/3/action/package_search?q=organization:hhs-gov");

		results = json.getJSONObject("result").getJSONArray("results");
		for (int i = 0; i < results.length(); i++) {
			...
			DataPackage dataPackage = new DataPackage(name, id, title, total,
					recent, tags);
